...
document.addEventListener("DOMContentLoaded", () => {
    const defaultBtn = document.querySelector('.sorting__btn');
console.log(defaultBtn.innerText);
    if (defaultBtn.innerText==="ЗА ЗАМОВЧУВАННЯМ") {
        defaultBtn.addEventListener('click', () => {
            // Скидання фільтрів
            document.querySelector('input[name="direction"][value="all"]').checked = true;
            document.querySelector('input[name="category"][value="all"]').checked = true;

            // Рендеримо початковий масив
